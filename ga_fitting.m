function fits = ga_fitting(varargin)
    if ~isempty(varargin)
        for i=1:length(varargin)
            if strcmp(varargin{i},'InitialGuess')
                initialPars=varargin{i+1};
            elseif strcmp(varargin{i},'ParameterName')
                parsName=varargin{i+1};
            elseif strcmp(varargin{i},'KpFunction') %Kp_fun(yModel,fitPars)
                parsName=varargin{i+1};   
            end
        end
    else
    ...
    for i=1:size(Kp_exp,2)
        ...
        options=optimoptions('ga','TolFun',1e-9);
            
        if exist('initialPars','var')
            options.InitialPopulationMatrix=initialPars(iCount).pars;
        end
        
        SSR_fun = residual(y_exp,Kp_exp(:,iCount));
        
        [fitPars_min,SSR_min] = ga(SSR_fun,nPars,[],[],[],[],LB,UB,...
            nlcon_fun(y_exp,Kp_exp(:,i),userVals(:,i),Kp_fun),options);
        ...
        [Kp,dqdP,q] = Kp_fun(yModel,fitPars);
        fprintf( ...
  '\nTrial: %s \nSSR=%.4g \n[B1,B2,B3,C1,C2,C3]=[%s] \ndqdP>0: %s \nq(y=0)=%.4g \nq(y=1)=%.4g \n',...
...
% Returns a function handle to be used in ga(...) 
% inside the ga_fitting function.
function fun = nlcon_fun(y_exp,Kp_exp,q_pure,Kp_fun)
